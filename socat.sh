...
config_socat(){
    echo -e "${Green}遇到技术问题请联系考拉微信：15671942${Font}"
    echo -e "${Green}考拉提示请输入Socat配置信息！${Font}"
    read -p "考拉提示请输入本地端口:" port1
    read -p "考拉提示请输入远程端口:" port2
    read -p "考拉提示请输入远程IP:" socatip
}

start_socat(){
    echo -e "${Green}考拉提示正在配置Socat...${Font}"
...
install_socat(){
    echo -e "${Green}考拉提示即将安装Socat...${Font}"
    if [ "${OS}" == 'CentOS' ];then
        yum install -y socat
    else
        apt-get -y update
    ...
    echo -e "${Green}考拉提示Socat安装完成！${Font}"
    fi
}
